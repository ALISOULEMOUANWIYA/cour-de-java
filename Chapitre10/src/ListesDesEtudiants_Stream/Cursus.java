		...
		for(Etudiant MonCle : liste) {
			if(MonCle.getMatriculeEtud().equalsIgnoreCase(MatriculeEtud)) {
				// creation d'un flux de traitement
				  liste.stream() // pour chaque element du flux, recherche ceux commençant par "A"
				  .filter(e -> e.getMatriculeEtud().startsWith(MatriculeEtud))
				  .forEach(e -> e.afficherUnEtudiant());// Pour element commençant par "A"
			}
		}	
	}
	
	public void rechercheMajore() {
		Etudiant max = liste.stream()
				.max((e1,e2) -> (e1.getMoyenne()) - e2.getMoyenne())
				.get();
		max.afficherUnEtudiant();
	}
	
	public void memeprenom(String tmp) {
		liste.stream()
		.filter(e -> e.getNomEtud().startsWith(tmp))
		.forEach(e -> e.afficherUnEtudiant());
	}
